...
local function combatSetupAimPunch(char)
    ...
    combatAimConnection = anim.AnimationPlayed:Connect(function(track)
        ...
        if combatS.aimPunchActive and combatTrackedPunchAnimations[animId] then
            -- Save original state HERE, before any RenderStepped elapsed check can fire.
            -- Previously this was saved on the first RenderStepped frame, which meant a low
            -- Duration slider value (or a slow frame) could trigger the restore branch while
            -- both vars were still nil — leaving AutoRotate permanently false.
...
svc.Run.RenderStepped:Connect(function()
    if not combatS.aimPunchActive then
        if combatPunchAiming then
            ...
            local hum  = char and char:FindFirstChild("Humanoid")
            local hrp  = char and char:FindFirstChild("HumanoidRootPart")
            if hrp and combatOriginalHRPRotY ~= nil then
                hrp.CFrame = CFrame.new(hrp.Position) * CFrame.fromEulerAnglesYXZ(0, combatOriginalHRPRotY, 0)
                combatOriginalHRPRotY = nil
            end
            if hum and combatOriginalAutoRotate ~= nil then
                hum.AutoRotate = combatOriginalAutoRotate
    ...
    if elapsed > combatS.aimPunchDuration then
        ...
        if combatOriginalAutoRotate ~= nil then
            hum.AutoRotate = combatOriginalAutoRotate
            combatOriginalAutoRotate = nil
        end
        return
...
local function abDotDestroy()
    if _abDotGui then
        pcall(function() _abDotGui:Destroy() end)
        _abDotGui = nil; _abDotBtn = nil
    end
end
