...
• HDT remade to use FINAL AUTO BLOCK logic with LinearVelocity (no MoveTo, no BodyVelocity)
...
-- Logic from FINAL_AUTO_BLOCK (Heartbeat-driven BodyVelocity, debounce, 0.4s timeout)
-- Keeps Hutao's BLOCK_ANIMS IDs and combatS settings
local combatHDTLastTime = 0
local HDT_CD = 0.5
local _combatHDTDebounce = false

-- Helper to resolve the killer's root part
...
local function combatHDTBeginDrag(killerModel, blockTrack)
    -- Basic guards (FINAL_AUTO_BLOCK style)
    ...
    -- save old locomotion state
    local oldWalk = humanoid.WalkSpeed
    local oldJump = humanoid.JumpPower
    local oldPlatformStand = humanoid.PlatformStand

    -- zero walk/jump so humanoid doesn't fight the BodyVelocity
    humanoid.WalkSpeed = 0
    humanoid.JumpPower = 0
    humanoid.PlatformStand = false

    -- BodyVelocity — horizontal only (keeps gravity)
    local bv = Instance.new("BodyVelocity")
    bv.MaxForce = Vector3.new(1e5, 0, 1e5)
    bv.Velocity = Vector3.new(0, 0, 0)
    bv.Parent = hrp

    -- Heartbeat connection drives movement each frame (FINAL_AUTO_BLOCK style)
    local conn
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        local speed = combatS.hdtMoveSpeed or 5.6
    ...
    -- delay before starting (uses combatS.hdtDelay)
    if combatS.hdtDelay and combatS.hdtDelay > 0 then
        task.wait(combatS.hdtDelay)
    end

    -- timeout safety (FINAL_AUTO_BLOCK uses 0.4s)
    task.delay(0.4, function()
        if _combatHDTDebounce then
            _combatHDTDebounce = false
        end
    end)
end
...
sec_017:Input({ Title = "HDT speed", Flag = "combatHDTMoveSpeed", Default = tostring(combatS.hdtMoveSpeed), Numeric = true, Placeholder = "5.6", Callback=function(v) combatS.hdtMoveSpeed = tonumber(v) or combatS.hdtMoveSpeed end
})

sec_017:Input({ Title = "HDT delay", Flag = "combatHDTDelay", Default = tostring(combatS.hdtDelay), Numeric = true, Placeholder = "0", Callback=function(v) combatS.hdtDelay = tonumber(v) or combatS.hdtDelay end
})
