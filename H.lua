...
------------------------------------------------------------------------
local secDocESP = tabVisual:Section({ Title = "Jane doe Document ESP", Opened = true })
local docESP = { on = false }
local foundDoc = false

local function makeDocESP(obj)
    if foundDoc then return end
    ...
    obj.AncestryChanged:Connect(function()
        if not obj.Parent then foundDoc = false end
    end)
end

local function docESPStart()
    foundDoc = false
    for _, v in ipairs(workspace:GetDescendants()) do
        if not foundDoc then makeDocESP(v) end
    end
    workspace.DescendantAdded:Connect(function(v)
        if not docESP.on or foundDoc then return end
        task.wait(0.1)
        makeDocESP(v)
    end)
end

local function docESPStop()
    foundDoc = false
    for _, v in ipairs(workspace:GetDescendants()) do
        local h = v:FindFirstChild("CuteDocESP"); if h then h:Destroy() end
        local b = v:FindFirstChild("CuteBillboard"); if b then b:Destroy() end
    end
end

secDocESP:Toggle({ Title = "🌸 Document ESP", Type = "Checkbox", Flag = "docESPOn", Default = docESP.on,
    Callback = function(on)
        docESP.on = on
        if on then docESPStart() else docESPStop() end
    end
})
