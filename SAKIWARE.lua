...
local function aiStartMove()
    ...
    aiState.moveConn = svc.Run.Heartbeat:Connect(function()
        ...
        -- If close, discard stale waypoints and track live position directly
...
local function combatLockOnPunch()
    ...
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    local hum  = char and char:FindFirstChildOfClass("Humanoid")
    if not hrp or not hum then _clpActive = false; return end
    local killer = combatGetNearestKiller()
    local khrp   = killer and combatGetKillerHRP(killer)
    if not khrp then _clpActive = false; return end
    ...
    conn = svc.Run.Heartbeat:Connect(function()
        if not (hrp and hrp.Parent) or not (khrp and khrp.Parent) then
            ...
            if hum and hum.Parent then hum.AutoRotate = oldAR end
            _clpActive = false
            return
        end
        bg.CFrame = CFrame.lookAt(hrp.Position, Vector3.new(khrp.Position.X, hrp.Position.Y, khrp.Position.Z))
...
sec_015:Dropdown({ Title = "Block Type", Values = {"Block","Charge","7n7 Clone"}, Default = combatS.blockType, Callback=function(v) combatS.blockType=v end 
})

sec_015:Slider({ Title = "Detection Range", Value = {Min=5,Max=50,Default=combatS.detectionRange}, Step = 1, Callback=function(v) combatS.detectionRange=v end 
})



sec_015:Slider({ Title = "Block Delay (s)", Value = {Min=0,Max=0.5,Default=combatS.blockDelay}, Step = 0.01, Callback=function(v) combatS.blockDelay=v end 
})

sec_015:Toggle({ Title = "Double Block Tech", Default = combatS.doubleBlock, Callback=function(on) combatS.doubleBlock=on end, Type = "Checkbox"})

sec_015:Toggle({ Title = "Anti-Bait", Default = combatS.antiBait, Callback=function(on) combatS.antiBait=on end, Type = "Checkbox"})

sec_015:Slider({ Title = "Block Miss Chance %", Value = {Min=0,Max=100,Default=combatS.abMissChance}, Step = 1, Callback=function(v) combatS.abMissChance=v end 
})

local sec_016 = tabGuest1337:Section({ Title = "Auto Punch", Opened = true })

sec_016:Toggle({ Title = "Auto Punch", Default = combatS.autoPunchOn, Callback=function(on) combatS.autoPunchOn=on end, Type = "Checkbox"})

local sec_017 = tabGuest1337:Section({ Title = "HDT (Hitbox Dragging)", Opened = true })


sec_017:Toggle({ Title = "Enable HDT", Default = combatS.hdtEnabled, Callback=function(on) combatS.hdtEnabled=on end, Type = "Checkbox"})

sec_017:Slider({ Title = "HDT Speed", Value = {Min=1,Max=30,Default=combatS.hdtSpeed}, Step = 0.5, Callback=function(v) combatS.hdtSpeed=v end 
})

sec_017:Slider({ Title = "HDT Delay (s)", Value = {Min=0,Max=0.5,Default=combatS.hdtDelay}, Step = 0.01, Callback=function(v) combatS.hdtDelay=v end 
})

sec_017:Slider({ Title = "HDT Miss Chance %", Value = {Min=0,Max=100,Default=combatS.hdtMissChance}, Step = 1, Callback=function(v) combatS.hdtMissChance=v end 
})
...
sec_018:Toggle({ Title = "Facing Check", Default = combatS.facingCheck, Callback=function(on) combatS.facingCheck=on end, Type = "Checkbox"})
...
sec_019:Toggle({ Title = "Lock On Punch", Default = combatS.charLockOn, Callback=function(on) combatS.charLockOn=on end, Type = "Checkbox"})

sec_019:Slider({ Title = "Lock Duration (s)", Value = {Min=0.05,Max=1.0,Default=combatS.lockDuration}, Step = 0.05, Callback=function(v) combatS.lockDuration=v end
})

sec_019:Slider({ Title = "Lock Max Distance", Value = {Min=5,Max=100,Default=combatS.lockMaxDist}, Step = 5, Callback=function(v) combatS.lockMaxDist=v end 
})

sec_019:Slider({ Title = "Prediction", Value = {Min=0,Max=15,Default=combatS.predictionVal}, Step = 0.5, Callback=function(v) combatS.predictionVal=v end 
})
