...
-- gradient text
------------------------------------------------------------------------
local function gradText(text, c1, c2)
    local out = ""
    local len = #text
    for i = 1, len do
        ...
        out = out .. string.format('<font color="rgb(%d,%d,%d)">%s</font>', r, g, b, text:sub(i,i))
    end
    return out
end
...
ui:Popup({
    Title   = gradText("SAKIWARE", Colors.Purple, Colors.LightBlue),
    Icon    = "sparkles",
    Content = gradText("The Ultimate Dead by Daylight Script", Colors.Gold, Colors.LightBlue)
        .. "<br/>"
        .. gradText("Discord: discord.gg/dwNhexu44z", Colors.Blue, Colors.Purple),
    Buttons = {
        { Title = "Exit",   Variant = "Tertiary",   Callback = function() end },
        { Title = "Copy Discord", Variant = "Secondary", Callback = function()
            setclipboard("https://discord.gg/dwNhexu44z")
            ui:Notify({ Title = "Copied!", Content = "Discord invite copied.", Icon = "check-circle", Duration = 3 })
            popupDone = true
        end },
        { Title = gradText("Continue", Colors.Toxic, Colors.Gold), Variant = "Primary", Callback = function() popupDone = true end },
    },
})
...
ui:AddTheme({
    ...
    Toggle          = Color3.fromHex("#9D4EDD"),
    ToggleBar       = Colors.Gold,
    Checkbox        = Color3.fromHex("#9D4EDD"),
    CheckboxIcon    = Colors.Gold,
    Slider          = Color3.fromHex("#9D4EDD"),
    SliderThumb     = Colors.Gold,
    WindowBackground= Color3.fromHex("#1E293B"),
})
ui:SetTheme("SakiTheme")
...
game:GetService("RunService").RenderStepped:Connect(function()
    pcall(function()
        FPSTag:Set({ Title = "FPS: " .. math.floor(1 / game:GetService("Stats").HeartbeatTimeMs * 1000) })
        PingTag:Set({ Title = "Ping: " .. math.floor(game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue()) .. "ms" })
    end)
end)
